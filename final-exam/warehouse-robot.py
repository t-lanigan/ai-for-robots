...
def plan(warehouse, dropzone, todo):
    grid = add_walls(warehouse)
    cost = 0
    dropzone[0], dropzone[1] = dropzone[0]+1, dropzone[1]+1
    for box in todo:
        goal = find_box(box, grid)
        start = dropzone
        c, x, y = shortest_path(grid, start, goal)
        cost += c
        grid[x][y] = 0
        start = [x,y]
        goal = dropzone
...
def shortest_path(grid, start, goal):
    ...
    while frontier:
        ...
        succ = successors(state, goal, grid)
        for s in succ:
            if is_goal(s, goal):
                return s
            cost, x, y = s
...
def add_walls(grid):
    ...
    y = len(grid[0])
    walls = []
    walls.append(['*'for i in range(y+2)])
    for i in range(x):
        walls.append(['*'])
        for j in range(y):
            walls[i+1].append(grid[i][j])
        walls[i+1].append('*')
    walls.append(['*'for i in range(y+2)])
    return walls
...
movements = [[1.5,-1,-1],[1,-1,0],[1.5,-1,1],[1,0,1],[1,0,-1],[1.5,1,-1],[1,1,0],[1.5,1,1]]

def successors(state, goal, grid):
    cost, [x, y] = state
    succ = []
    for m_cost, dx, dy in movements:
        nx, ny = x+dx, y+dy
        if grid[nx][ny] == 0 or grid[nx][ny] == grid[goal[0]][goal[1]]:
            succ.append([cost+m_cost, nx, ny])
    return succ
...
print plan(warehouse3, dropzone3, todo3)